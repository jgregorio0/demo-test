...
class CreateCenterControllerTest {
  ...
  void create_shouldCreateCenter_whenRequestIsValid() throws Exception {
    ...
    then(creationMapper).fromRequest(any(CenterCreationRequest.class));
    then(centerService).create(any(CenterCreation.class));
    then(mapper).toResponse(createdCenter);
  }
  ...
  void create_shouldReturnBadRequest_whenClientIdIsNull() throws Exception {
    ...
    verify(centerService).should(never()).create(any(CenterCreation.class));
  }
